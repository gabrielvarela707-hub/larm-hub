...
const NAV: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      {
        label: "Atualização de Índices",
        href: "/cadastros/atualizacao-indices",
        moduleIds: ["cadastros_indices_economicos"],
      },
      {
        label: "Clientes",
        href: "/cadastros/clientes",
        moduleIds: ["cadastros_clientes"],
      },
      {
        label: "Fornecedores",
        href: "/cadastros/fornecedores",
        moduleIds: ["fin_fornecedores"],
      },
      {
        label: "Plano de Contas",
        href: "/cadastros/plano-contas",
        moduleIds: ["cadastros_plano_contas"],
      },
      {
        label: "Tipo de Documento",
        href: "/cadastros/tipo-documento",
        moduleIds: ["cadastros_tipo_documento"],
      },
    ],
  },
  {
    ...
    children: [
      {
        ...
        moduleIds: ["fin_cashflow"],
      },
      {
        label: "Orçamento",
        href: "/financeiro/orcamento",
        moduleIds: ["fin_orcamento"],
      },
      {
        label: "Mov. Orçado",
        href: "/financeiro/movimento-orcado",
        moduleIds: ["fin_orcamento"],
      },
      {
        label: "Mov. Bancario",
        href: "/financeiro/movimento",
        moduleIds: ["fin_movimento"],
      },
      {
        label: "Contas a Receber",
        href: "/financeiro/receber",
        moduleIds: ["fin_receber"],
      },
      {
      ...
      },
      {
        ...
        moduleIds: ["fin_bancos"],
      },
      {
        label: "Boletos",
        href: "/financeiro/boletos",
        moduleIds: ["fin_boletos"],
      },
      {
  ...
  { label: "Obras", href: "/obras", icon: HardHat, moduleIds: ["obras"] },
  {
    ...
    href: "/relatorios",
    icon: BarChart3,
    moduleIds: ["relatorios"],
    children: [
      {
        label: "Mapa de Vendas",
        href: "/relatorios/mapa-vendas",
        moduleIds: ["relatorios_mapa_vendas"],
      },
      {
        label: "Projetos / Obras",
        href: "/relatorios/projetos-obras",
        moduleIds: ["relatorios_projetos_obras"],
      },
      {
        label: "Unidades do Estoque",
        href: "/relatorios/unidades-estoque",
        moduleIds: ["relatorios_unidades_estoque"],
      },
      {
        label: "Implantacao / Plantas",
        href: "/relatorios/implantacao",
        moduleIds: ["relatorios_implantacao"],
      },
    ],
  },
...
const BOTTOM_NAV: NavItem[] = [
  {
    ...
    href: "/configuracoes",
    icon: Settings,
    moduleIds: ["configuracoes", "usuarios"],
  },
...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  function visibleChildren(item: NavItem) {
    return (item.children || []).filter((child) =>
      hasModuleAccess(user, child.moduleIds, "read"),
    );
  }
  ...
  function itemVisible(item: NavItem) {
    if (item.children?.length)
      return (
        ...
        hasModuleAccess(user, item.moduleIds, "read")
      );
  ...
  function isGroupActive(item: NavItem) {
    return (
      visibleChildren(item).some((c) => isActive(c.href)) ||
    ...
    >
                  ...
                  {!collapsed && (
                    <>
                      ...
                      {item.badge && (
                        <span
                          className={cn(
                            "text-[10px] text-white px-1.5 py-0.5 rounded-full leading-none",
      ...
      <div className="border-t border-white/[0.06] px-2 py-3 space-y-0.5">
        ...
        <button
          ...
          onClick={() => logout()}
          className={cn(
            "w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg cursor-pointer hover:bg-white/5 transition-colors mt-1",
        ...
        >
          ...
          {!collapsed && (
            <LogOut className="w-3.5 h-3.5 text-white/40 flex-shrink-0" />
          )}
        </button>
      </div>
    </aside>
