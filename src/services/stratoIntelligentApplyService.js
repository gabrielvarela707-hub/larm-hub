/**
 * Aplicação transacional da análise inteligente Strato — v0.6.6.
 *
 * Regras de segurança:
...
function stableSelectionKey(filename, analysisItem, parcelAnalysis) {
  ...
  const parcelIdentity = parcelAnalysis?.parcela?.id || [
    row.obra || '',
    row.unidade || '',
    row.parcela || '',
...
async function applyStratoIntelligentReturn({
  ...
  for (let itemIndex = 0; itemIndex < (analysis.items || []).length; itemIndex += 1) {
    ...
    for (let parcelIndex = 0; parcelIndex < (analysisItem.parcelas || []).length; parcelIndex += 1) {
      ...
      const key = stableSelectionKey(filename, analysisItem, parcelAnalysis)
      const wantsApply = selected.has(key)
      if (wantsApply) consumedSelections.add(key)
