...
router.post('/financeiro/contas-receber/bradesco/retorno', async (req, res) => {
  ...
  const intelligentSelections = Array.isArray(req.body?.selecoes_inteligentes)
    ? req.body.selecoes_inteligentes.map(value => String(value))
    : []
  ...
  try {
    ...
    if (applyPayment && applyIntelligent) {
      ...
      const appliedSummary = intelligentResults.reduce((total, result) => {
        ...
        total.bloqueios += Array.isArray(result.bloqueios_aplicacao) ? result.bloqueios_aplicacao.length : 0
        return total
      }, { parcelas: 0, movimentos: 0, valor: 0, bloqueios: 0 })
      ...
      await logAudit({
        ...
        meta: {
          ...
          bloqueios: appliedSummary.bloqueios,
          versao: '0.6.4',
        },
      }).catch(() => {})
      ...
      return res.json({
        ...
        message: appliedSummary.parcelas
          ? `${appliedSummary.parcelas} parcela(s) aplicada(s), com ${appliedSummary.movimentos} Movimento(s) Bancário(s) individual(is).`
          : 'Nenhuma nova baixa foi necessária; as parcelas selecionadas já estavam conciliadas ou permaneceram bloqueadas.',
        data: {
