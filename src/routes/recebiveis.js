 ...
 * v0.4.2 — Contratos, parcelas e baixas sempre vinculadas ao Movimento Bancário.
 */
...
const { analyzeStratoReturnReport, normalizeFilename: normalizeReturnFilename } = require('../services/stratoReturnReportAnalyzer')
...
// O campo opcional `relatorios_strato` recebe PDFs/imagens para extração assistida
// por IA. A IA somente estrutura o relatório; a conciliação e a baixa continuam
// sujeitas às regras determinísticas e aos limites de confiança do backend.
      ...
      if (!apiKey) {
        return res.status(422).json({
          ...
          message: provider === 'gemini'
            ? 'Configure a Gemini API Key em Configurações > Credenciais para analisar o relatório Strato.'
            : 'Configure a OpenAI API Key em Configurações > Credenciais para analisar o relatório Strato.',
        })
      }

      // A chamada externa ocorre antes da transação financeira. Assim, uma falha
      // de IA não mantém bloqueios no banco nem deixa baixas parcialmente gravadas.
      for (const report of reports) {
        analyzedReports.push(await analyzeStratoReturnReport({
          provider,
          apiKey,
          model,
