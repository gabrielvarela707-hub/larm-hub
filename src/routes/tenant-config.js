...
const { DEFAULT_AI_MODELS, listAiModels, normalizeProvider } = require('../services/aiModelService')
  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        geminiModel:        c.gemini_model         || DEFAULT_AI_MODELS.gemini,
        // Outros
...
router.put('/tenant-config', async (req, res) => {
  ...
  const mapped = {
    ...
    gemini_model:          b.geminiModel ? String(b.geminiModel).trim().replace(/^models\//i, '').slice(0, 120) : undefined,
    // Outros
...
router.post('/tenant-config/ai-models', async (req, res) => {
  const { tenant_id, role } = req.user
