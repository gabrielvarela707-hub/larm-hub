...
export default function BancosPage() {
  ...
  async function loadLancamentos(contaId: number) {
    setLancLoading(true)
    try {
      const r = await apiClient.get(`/financeiro/bancos/${contaId}/lancamentos`)
  ...
  function onBancoChange(val: string) {
    const found = BANCOS_BR.find(b => `${b.cod} - ${b.nome}` === val)
  ...
  function closeLancModal() { setShowLancModal(false); setLancContaId(null); setLancamentos([]) }
  ...
  async function deleteLancamento(id: number) {
    try {
      await apiClient.delete(`/financeiro/bancos/${lancContaId}/lancamentos/${id}`)
      await loadLancamentos(lancContaId!)
      load()
    } catch { }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Contas bancárias, saldos e movimentações</p>
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-[#0d1b2a] text-white">
                {['Empresa','Banco','Agência','Conta','Tipo','Saldo','Data Saldo','Status','Ações'].map(h => (
                  <th key={h} className="text-left px-3 py-2.5 font-semibold whitespace-nowrap text-xs">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {!loading && lista.length === 0 && (
                <tr><td colSpan={9} className="text-center text-slate-400 py-12 text-sm">
              ...
              {!loading && lista.map(b => (
                <tr key={b.id} className={cn('border-b border-slate-50 hover:bg-slate-50 transition-colors', !b.ativo && 'opacity-50')}>
                  ...
                  <td className="px-3 py-2 text-slate-500">{b.agencia || '—'}</td>
                  <td className="px-3 py-2 text-slate-500">{b.conta}{b.digito ? `-${b.digito}` : ''}</td>
                  ...
                  <td className="px-3 py-2 flex items-center gap-1">
                    <button onClick={() => openLancamentos(b)} title="Lançamentos extras"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mb-8">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">* campos obrigatórios</p>
              </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Empresa" name="empresa" required errors={errors}>
                  <select className={inp} value={form.empresa || ''}
                ...
                <Field label="Banco" name="banco_nome" required errors={errors}>
                  <select className={inp}
                    value={BANCOS_BR.find(b => b.nome === form.banco_nome)
                ...
                <Field label="Código do Banco" name="codigo_banco" errors={errors}>
                  <input className={inp} value={form.codigo_banco || ''}
                    onChange={e => set('codigo_banco', e.target.value)} placeholder="000" />
                </Field>

                <Field label="Tipo de Conta" name="tipo_conta" errors={errors}>
                  <select className={inp} value={form.tipo_conta || 'Corrente'}
                ...
                <Field label="Conta" name="conta" errors={errors}>
                  <input className={inp} value={form.conta || ''}
                    onChange={e => set('conta', e.target.value.replace(/\D/g, ''))}
                    inputMode="numeric" placeholder="000000" />
                </Field>

                <Field label="Dígito" name="digito" errors={errors}>
                  <input className={inp} value={form.digito || ''}
                    onChange={e => set('digito', e.target.value)} placeholder="0" maxLength={2} />
                </Field>
              ...
              <p className="text-[10px] text-slate-400 -mt-1">
                Saldo inicial e data são opcionais — podem ser inseridos depois via lançamentos extras (ícone <span className="font-semibold">$</span> na lista).
              </p>
            </div>

            <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
              <button onClick={closeForm} className="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50">Cancelar</button>
              <button onClick={save} disabled={saving}
                ...
                {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Salvando…</> : <><Check className="w-4 h-4" /> {editId ? 'Atualizar' : 'Salvar'}</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {showLancModal && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-start justify-center p-4 pt-10 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl mb-8">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div>
                <h2 className="text-base font-bold text-slate-800">Lançamentos da Conta</h2>
                <p className="text-xs text-slate-500 mt-0.5">{lancContaNome}</p>
              </div>
              <button onClick={closeLancModal} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400"><X className="w-4 h-4" /></button>
            </div>

            <div className="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
              {/* Formulário de novo lançamento */}
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-3">
                <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Novo Lançamento</p>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex flex-col gap-1 col-span-2">
                    ...
                    <input className={cn(inp, lancErrors.descricao && 'border-red-300')}
                    ...
                    {lancErrors.descricao && <p className="text-[10px] text-red-500">{lancErrors.descricao}</p>}
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-slate-600">
                      Valor (R$) *
                      <span className="text-slate-400 font-normal ml-1">
                        {lancForm.tipo === 'taxa' ? '(negativo = débito)' : '(positivo = crédito)'}
                      </span>
                    </label>
                    <input className={cn(inp, lancErrors.valor && 'border-red-300')}
                      type="number" step="0.01"
                      value={lancForm.valor || ''}
                      onChange={e => setLancForm(p => ({ ...p, valor: parseFloat(e.target.value) || 0 }))}
                      placeholder={lancForm.tipo === 'taxa' ? '-25.00' : '150.00'} />
                    {lancErrors.valor && <p className="text-[10px] text-red-500">{lancErrors.valor}</p>}
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-slate-600">Data *</label>
                    <input className={cn(inp, lancErrors.data && 'border-red-300')}
                      type="date" value={lancForm.data}
                      onChange={e => setLancForm(p => ({ ...p, data: e.target.value }))} />
                    {lancErrors.data && <p className="text-[10px] text-red-500">{lancErrors.data}</p>}
                  </div>
                  <div className="flex flex-col gap-1 col-span-2">
                    <label className="text-xs font-medium text-slate-600">Obs</label>
                    <input className={inp} value={lancForm.obs || ''}
                      onChange={e => setLancForm(p => ({ ...p, obs: e.target.value }))}
                      placeholder="Observação opcional" />
                  </div>
                </div>
                <button onClick={saveLancamento} disabled={lancSaving}
                  ...
                  {lancSaving ? <><Loader2 className="w-4 h-4 animate-spin" /> Salvando…</> : <><Plus className="w-4 h-4" /> Adicionar</>}
                </button>
              </div>
              ...
              {lancLoading ? (
                ...
                <p className="text-sm text-slate-400 text-center py-6">Nenhum lançamento nesta conta ainda.</p>
              ) : (
                <div className="space-y-2">
                  {lancamentos.map(l => (
                    <div key={l.id} className="flex items-center gap-3 bg-white rounded-xl border border-slate-100 px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className={cn('text-[10px] px-1.5 py-0.5 rounded-full font-medium', TIPO_LANC_COLORS[l.tipo])}>
                      ...
                      <p className={cn('text-sm font-bold tabular-nums', l.valor >= 0 ? 'text-green-700' : 'text-red-600')}>
                        {l.valor >= 0 ? '+' : ''}{R$(l.valor)}
                      </p>
