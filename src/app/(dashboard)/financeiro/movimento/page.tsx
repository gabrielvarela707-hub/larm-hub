...
import { Search, TrendingUp, TrendingDown, DollarSign, ChevronLeft, ChevronRight, ArrowUpDown } from 'lucide-react'
...
interface Mov {
  ...
  conta_contabil: string; natureza_financeira: string
  tipo_lancamento: string; vencimento: string | null
  dia: number; mes: number; ano: number
}
interface Filtros { empresas: string[]; bancos: string[]; anos: number[] }
...
export default function MovimentoPage() {
  ...
  const [fTipoLancamento, setFTipoLancamento] = useState('financeiro')
  const [fOrdenar,        setFOrdenar]        = useState('data_desc')
  const [busca,           setBusca]           = useState('')

  useEffect(() => {
    apiClient.get('/financeiro/movimento/filtros').then(r => setFiltros(r.data.data)).catch(() => {})
  }, [])

  const loadMovs = useCallback(async (pg = 1) => {
    setLoading(true)
    try {
      const params: Record<string, unknown> = { page: pg, limit: LIMIT, ano: fAno, ordenar: fOrdenar }
      if (fEmpresa)        params.empresa         = fEmpresa
      if (fBanco)          params.banco           = fBanco
      if (fMes)            params.mes             = fMes
      if (fTipo)           params.tipo            = fTipo
      if (busca)           params.busca           = busca
      if (fTipoLancamento) params.tipo_lancamento = fTipoLancamento
  ...
  }, [fEmpresa, fBanco, fAno, fMes, fTipo, fTipoLancamento, fOrdenar, busca])
  ...
  }, [fAno, fEmpresa, fTipoLancamento])

  useEffect(() => {
    if (activeTab === 'lista') loadMovs(1)
    else loadResumo()
  }, [fEmpresa, fBanco, fAno, fMes, fTipo, fTipoLancamento, fOrdenar, activeTab])
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Movimentações financeiras por empresa e banco</p>
        </div>
      ...
      {/* Filtros */}
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm px-4 py-3 space-y-3">
        ...
        {/* Separação Financeiro vs Administrativo + Ordenação */}
        <div className="flex flex-wrap gap-3 items-center border-t border-slate-50 pt-2.5">
          <span className="text-xs text-slate-500 font-medium">Exibir:</span>
          {[
            { v: 'financeiro',     l: '📊 Somente Financeiro' },
            { v: 'administrativo', l: '⚙️ Administrativo' },
            { v: '',               l: 'Todos' },
          ].map(o => (
            <button key={o.v} onClick={() => setFTipoLancamento(o.v)}
            ...
            <div className="flex gap-2">
              <div className="relative">
                ...
                <input value={busca} onChange={e => setBusca(e.target.value)}
                  ...
                  className="pl-9 pr-4 py-2 w-72 text-sm bg-white border border-slate-200 rounded-lg text-slate-700 focus:outline-none focus:border-blue-400" />
              </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-[#0d1b2a] text-white">
                    {['Data','Empresa','Banco','Fornecedor / Histórico','Conta','Venc.','Entradas','Saídas','Saldo'].map(h => (
                ...
                <tbody>
                  ...
                  {!loading && movs.length === 0 && (
                    <tr><td colSpan={9} className="text-center text-slate-400 py-12 text-sm">Nenhuma transação encontrada</td></tr>
                  )}
                  {!loading && movs.map(m => (
                    <tr key={m.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-slate-400 text-[10px]">{fmtDate(m.vencimento)}</td>
              ...
              <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
                ...
                <div className="space-y-2">
                  {resumo.top_despesas.map((d, i) => {
                    const pct = (d.total / resumo.top_despesas[0].total) * 100
                    return (
