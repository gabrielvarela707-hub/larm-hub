  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'A receber este mês', value: kpis.monthlyReceivable, color: 'text-slate-900', bg: 'bg-blue-50', icon: DollarSign },
          { label: 'Em atraso', value: kpis.overdueReceivable, color: 'text-red-600', bg: 'bg-red-50', icon: AlertCircle },
          { label: 'Recebido (mês)', value: kpis.monthlyReceivable - kpis.overdueReceivable, color: 'text-emerald-600', bg: 'bg-emerald-50', icon: CheckCircle },
          { label: 'Inadimplência', value: kpis.defaultRate, color: 'text-amber-600', bg: 'bg-amber-50', icon: Clock },
        ].map(k => (
          <div key={k.label} className="bg-white border border-slate-100 rounded-xl p-4 shadow-card">
            ...
            <p className={cn('text-lg font-semibold', k.color)}>
              {k.label === 'Inadimplência' ? `${k.value}%` : formatCurrency(k.value)}
            </p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por cliente ou contrato..." className="text-sm outline-none w-48 placeholder:text-slate-400" />
                ...
                <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      {p.boletoUrl && (
                        <a href={p.boletoUrl} className="text-xs text-blue-600 hover:underline">Boleto</a>
                      )}
                      {p.status !== 'paga' && (
                        <button className="text-xs text-emerald-600 hover:underline">Baixar</button>
                    </div>
                </tr>
              )
            })}
