...
function KpiCard({
  label, value, change, icon: Icon, color, format = 'currency',
}: {
  label: string
  value: number
  change: number
  ...
  const recentLeads = mockLeads.slice(0, 5)
    <div className="space-y-6 max-w-[1400px]">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Bom dia, Fernando 👋</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard label="Inadimplência"         value={kpis.defaultRate}     change={kpis.defaultRateChange} icon={AlertCircle} color="bg-amber-500"   format="percent" />
            ...
            {overdue.length > 0 && (
              <>
                ...
                <div>
                  ...
                  {overdue.length > 2 && (
                    <a href="/financeiro/receber" className="text-xs text-blue-600 hover:underline">
                      +{overdue.length - 2} em atraso
                    </a>
                  )}
              </>
            )}
