...
import {
  MapPin, ZoomIn, ZoomOut, ChevronDown, X,
  Clock, TrendingUp, Eye, User,
} from 'lucide-react'
import { cn, formatCurrency } from '@/lib/utils'
import { useGoogleMaps } from '@/lib/use-google-maps'
import {
  MAP_EMPREENDIMENTOS, LOT_FILL_COLORS, LOT_STROKE_COLORS,
  STATUS_LABELS, type MapLot, type MapEmpreendimento,
} from '@/lib/map-data'
    ...
    <div className="absolute top-14 right-3 w-68 bg-white rounded-2xl shadow-dialog border border-slate-100 overflow-hidden animate-fade-in z-20">
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className={cn('text-xs font-semibold px-2.5 py-1 rounded-full border', statusColor[lot.status])}>
...
function StatsBar({ lots }: { lots: MapLot[] }) {
  ...
  const disp  = lots.filter(l => l.status === 'disponivel').length
  const res   = lots.filter(l => l.status === 'reservado').length
  const vend  = lots.filter(l => l.status === 'vendido').length
    ...
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 w-full max-w-md px-3">
      <div className="bg-white/95 backdrop-blur rounded-2xl shadow-dialog border border-slate-100 px-4 py-3">
        <div className="flex h-1.5 rounded-full overflow-hidden gap-0.5 mb-3">
          ...
          <div className="bg-emerald-500 transition-all duration-500 rounded-full" style={{ width: `${pct(disp)}%` }} />
        </div>
        <div className="grid grid-cols-4 gap-2 text-center">
          {[
            { label: 'Total',       value: total, color: 'text-slate-800' },
            { label: 'Disponiveis', value: disp,  color: 'text-emerald-600' },
  ...
  function renderPolys(map: google.maps.Map, lotsArr: MapLot[], fil: LotStatus | 'all') {
    lotsArr.forEach(lot => {
      ...
      const poly = new google.maps.Polygon({
        ...
        fillColor:    LOT_FILL_COLORS[lot.status],
        fillOpacity:  0.65,
        strokeColor:  LOT_STROKE_COLORS[lot.status],
        strokeWeight: 1.5,
        strokeOpacity: 0.9,
        map,
      })
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-slate-200" style={{ height: 'calc(100vh - 90px)', minHeight: 600 }}>
            ...
            <button key={f.id} onClick={() => setFilter(f.id)}
              className={cn('flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold shadow-card border transition-colors',
                filter === f.id
                  ...
                  : f.id === 'disponivel' ? 'bg-emerald-600 text-white border-emerald-600'
                  : f.id === 'reservado'  ? 'bg-amber-500 text-white border-amber-500'
                  :                         'bg-red-500 text-white border-red-500'
                  : 'bg-white/95 text-slate-600 border-slate-200 hover:bg-white'
              )}>
              {f.id !== 'all' && (
                <span className="w-2 h-2 rounded-full" style={{ background: filter === f.id ? 'white' : LOT_FILL_COLORS[f.id as LotStatus] }} />
              )}
              {f.label}
              <span className="opacity-70 text-[10px]">
                ({f.id === 'all' ? lots.length : lots.filter(l => l.status === f.id).length})
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 z-10">
          <div className="text-center p-8 max-w-sm">
            ...
            <p className="text-sm text-slate-500 mb-5 leading-relaxed">
              Configure sua chave da Google Maps API para visualizar os lotes do empreendimento com polygonos coloridos por status.
            </p>
            <div className="bg-slate-900 rounded-xl px-4 py-3 text-left mb-3">
      ...
      <div className="absolute bottom-24 right-3 z-20 bg-white/95 backdrop-blur rounded-xl shadow-card border border-slate-100 px-3 py-2.5">
        ...
        {(['disponivel', 'reservado', 'vendido', 'bloqueado'] as LotStatus[]).map(s => (
          <div key={s} className="flex items-center gap-2 mb-1.5 last:mb-0">
            <span className="w-3 h-3 rounded-sm flex-shrink-0" style={{ background: LOT_FILL_COLORS[s] }} />
            <span className="text-xs text-slate-600">{STATUS_LABELS[s]}</span>
          </div>
        ))}
      </div>
