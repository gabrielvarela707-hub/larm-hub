  ...
  return (
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Automacoes de Marketing</h1>
          <p className="text-sm text-slate-500 mt-0.5">{active} de {total} ativas · {totalRuns.toLocaleString('pt-BR')} execucoes totais</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Total de execucoes', value: totalRuns.toLocaleString('pt-BR'), color: 'text-blue-600' },
          { label: 'Pausadas', value: total - active, color: 'text-slate-600' },
        ].map(k => (
      ...
      <div className="space-y-3">
        {automations.map(auto => (
          <div key={auto.id} className={cn('bg-white rounded-xl border shadow-card p-5 transition-all', auto.active ? 'border-slate-100' : 'border-slate-100 opacity-70')}>
            <div className="flex items-start justify-between gap-4 mb-4">
              <div className="flex items-start gap-3">
                <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0', auto.active ? 'bg-blue-100' : 'bg-slate-100')}>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[11px] text-slate-400">{auto.executionCount.toLocaleString('pt-BR')} execucoes</span>
                    {auto.lastRunAt && (
                      <span className="text-[11px] text-slate-400">Ultimo: {relativeTime(auto.lastRunAt)}</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-800">
        Os envios via WhatsApp utilizam a API oficial da Meta. Os custos sao cobrados diretamente pela Meta Platforms.
      </div>
    </div>
  )
}
