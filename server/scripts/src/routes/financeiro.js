...
const MOVIMENTO_ORCADO_WRITE_ROLES = new Set(['super_admin', 'admin', 'manager', 'controller', 'financial'])

function canInactivateMovimentoOrcado(user) {
  return Boolean(user?.role && MOVIMENTO_ORCADO_WRITE_ROLES.has(user.role))
}
...
router.patch('/orcamento/movimento/:id/inativar', async (req, res) => {
